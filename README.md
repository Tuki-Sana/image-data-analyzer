...
保存される JSON には **`schemaVersion`** があり、**パレットデータの形（スキーマ）が変わったときだけ**番号が上がります。**アプリ本体のバージョン（0.2.1 など）とは別物**です。いまの形式は **`schemaVersion: 1`** で、ルートに次のフィールドがあります。
...
`package.json`・`src-tauri/tauri.conf.json`・`src-tauri/Cargo.toml` の **version は同一の値**に揃えています（現在 **0.2.1**）。変更履歴は **`CHANGELOG.md`** を参照してください。
