...
この計算には、**テンプレートに必要な別の理想色相が欠けていても減点できない**という既知の制限があります。観測した各色相から最寄りの理想角までの距離だけを集計し、未使用の理想角を評価しないためです。そのため、補色・分割補色・トライアド・テトラードへの適合度とは扱いません。

値と `harmonyScores` フィールドは内部解析、および旧 `schemaVersion: 3 / 4` JSON の読み込み互換性のため保持しています。新しくコピー・保存する共有 JSON、ひと目サマリ、共有 PDFからは除外し、UIでは折りたたんだ実験的表示へ降格しています。新しい採点式はこの版では設計・実装していません。

---

## 9. 要約（gist）

[`build_analysis_gist`](../src-tauri/src/analyze.rs): 理論ブロックの**支配色相サマリ**と、Open/Tailwind の **ΔE2000（1〜3 位）**を、行ごとに `role`（`mono` / `label` / `body` / `foot`）付きで並べ、`gist_ja` に改行連結したテキストも生成します。実験的な色相配置値は要約へ含めません。
...
- **含める内容**: プレビュー画像、**ファイル名**・寸法・ファイル情報、**主要色**、**ひと目サマリ（gist 行）**、**Open Color / Tailwind 近似（ΔE2000）**、**WCAG コントラスト（主要色 1 位 vs 2 位）**、**色彩理論メモ（`TheoryBlock`・折りたたみ相当を常時展開）**、**EXIF**。元画像の絶対パスと実験的な色相配置値は共有 PDF へ出しません。
- **出力方式**: `html2canvas` で画面用 DOM をラスタ化し、ページごとの JPEG として PDF に配置します。テキスト主体のネイティブ PDF ではありません。
...
- [README.md](../README.md) … 利用者向け機能・用語の説明・既知の制限・開発手順・ドキュメント案内
