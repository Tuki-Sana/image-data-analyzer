...
4. **代表色**: 各クラスタに属したピクセルの **R・G・B をそれぞれ平均**（整数除算、0〜255 にクランプ）した色を表示用の支配色とします（Lab 重心の直接変換ではなく、**sRGB ガムット内**に留めるため）。
5. **同一 RGB のマージ**: 別クラスタでも平均 RGB が一致した行は **出現率（%）を合算**して 1 行にまとめます（単色画像で k=8 でも 1 支配色になるようにするため）。
