...
Teinte の**処理の流れ**と**主要ディレクトリ**だけをまとめたメモです。細部はコードとコメントを参照してください。
...
**画像や解析を受け付ける外部サーバーはありません。** 開発時は Vite がローカルでフロントを配信しますが、解析やファイルアクセスは **自分の PC 上**で完結します（ブラウザにアップロードしてサーバー側で処理する形ではありません）。
...
├── public/                # 静的アセット（Vite）
├── src/
│   ├── App.vue            # ルートレイアウト・composable 配線
│   ├── main.ts
...
│   │   ├── harmony.rs
│   │   └── meta.rs
│   │   # ※ 配色の役割分類（ベース/アソート/アクセント）は Rust 側なし → src/utils/colorRole.ts
